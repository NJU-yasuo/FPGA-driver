 ...
 * @LastEditTime: 2020-03-05 18:07:30
 */
#include <stdio.h>
#include "fx_hpca.h"
#include <stdbool.h>
...
bool set_18_verify_parameter(op_config_t *op_config, int block_id, int sub_id, int row, int col_1, int col_2)
{
    ...
    if (block_id == 1)
    {
        if (sub_id % 2 == 0)
        {
            sprintf(op_config->read_verify[0].filename, "block_op/resnet18_bin/block%d/relu_outb.tile_idx_%d_%d.bin", block_id, row, col_1 * 2 + col_2);
            sprintf(op_config->read_from_ddr[0].filename, "block_op/resnet18_bin/block%d/Block%d_Tile%d_row%d_col1_%d.bin", block_id, block_id, sub_id, row, col_1);
            ...
            // sprintf(op_config->read_from_ddr[1].filename, "block_op/resnet18_bin/block%d/Block%d_Tile%d_row%d_col1_%d_ram1.bin", block_id, block_id, sub_id, row, col_1);
            // op_config->read_from_ddr[1].ddr_base_address = op_config->value_at_reg_0x90 + op_config->value_at_reg_0x94;
            // op_config->read_from_ddr[1].info_size = op_config->value_at_reg_0x94;

            // sprintf(op_config->read_from_ddr[2].filename, "block_op/resnet18_bin/block%d/Block%d_Tile%d_row%d_col1_%d_ram2.bin", block_id, block_id, sub_id, row, col_1);
            // op_config->read_from_ddr[2].ddr_base_address = op_config->value_at_reg_0x90 + op_config->value_at_reg_0x94 * 2;
            // op_config->read_from_ddr[2].info_size = op_config->value_at_reg_0x94;
        }
    }
    else if (block_id == 0)
    {
        sprintf(op_config->read_verify[0].filename, "block_op/resnet18_bin/block%d/relu_outb.tile_idx_%d_%d.bin", block_id, row, col_1);
        sprintf(op_config->read_from_ddr[0].filename, "block_op/resnet18_bin/block%d/Block%d_Tile%d_row%d_col1_%d.bin", block_id, block_id, sub_id, row, col_1);
        op_config->read_from_ddr[0].ddr_base_address = op_config->value_at_reg_0x90;
        op_config->read_from_ddr[0].info_size = op_config->value_at_reg_0x94;
    }
...
void set_18_parameter(op_ini_param_t *op_param)
{
    ...
    if (op_param->v_layer == 2)
    {
        ...
        op_param->ResAddLayer = 2; // -1
        op_param->QuanValueOfSix = 96;
        op_param->SelOvRAMtoRead = 0;
        op_param->SelRelu6 = 0;
        // .DWConv = 0b0000,
        op_param->which_prebn_act_bank = 4;
    }
    else if (op_param->v_layer == 3)
    {
        ...
        op_param->output_buffer_seq[0] = 2; //{2, 0, 0, 0},
        op_param->output_buffer_seq[1] = 4; //{2, 0, 0, 0},
        op_param->output_buffer_seq[2] = 4; //{2, 0, 0, 0},
        op_param->output_buffer_seq[3] = 7; //{2, 0, 0, 0},

        op_param->act_bank_in_seq[0] = 3;  //{3, 3, 3, 3},
        op_param->act_bank_in_seq[1] = 3;  //{3, 3, 3, 3},
        op_param->act_bank_in_seq[2] = 1;  //{3, 3, 3, 3},
        op_param->act_bank_in_seq[3] = 3;  //{3, 3, 3, 3},
        op_param->act_bank_out_seq[0] = 1; //{1, 3, 3, 3},
        op_param->act_bank_out_seq[1] = 1; //{1, 3, 3, 3},
        op_param->act_bank_out_seq[2] = 1; //{1, 3, 3, 3},
        op_param->act_bank_out_seq[3] = 3; //{1, 3, 3, 3},
        ...
        op_param->SelRelu6 = 0;
        op_param->which_prebn_act_bank = 1;

        op_param->LastlayerStride = 1;
    }
}
    ...
    for (int col_1 = 0; col_1 < op_param->col / op_param->blockstride; col_1++)
    {
        for (int row = 0; row < op_param->row; row++)
        {
            for (int col_2 = 0; col_2 < op_param->blockstride; col_2++)
            {
                ...
                {
                    set_18_verify_parameter(&op_config, test_block, op_param->sub_id, row, col_1, col_2);
                }
