 ...
 * @LastEditTime: 2020-03-05 18:07:18
 */
#include <stdio.h>
#include "fx_hpca.h"
#include <stdbool.h>
...
bool set_mobilenet_verify_parameter(op_config_t *op_config, int block_id, int sub_id, int row, int col_1, int col_2)
{

    // sprintf(op_config->read_verify[0].filename, "block_op/task2_bin/relu_outb.tile_idx_%d_%d.3.bin", row, col_1);
    if (block_id == 0)
    {
        sprintf(op_config->read_verify[0].filename, "block_op/mobilenet_bin/block%d/relu_outb.tile_idx_%d_%d.1.bin", block_id, row, col_1);
        sprintf(op_config->read_from_ddr[0].filename, "block_op/mobilenet_bin/block%d/Block%d_Tile%d_row%d_col1_%d.bin", block_id, block_id, sub_id, row, col_1);
    ...
    else if (block_id == 1)
    {
        if (sub_id % 2 == 0)
        {
            sprintf(op_config->read_verify[0].filename, "block_op/mobilenet_bin/block%d/relu_outb.tile_idx_%d_%d.1.bin", block_id, row, col_1 * 2 + col_2);
            sprintf(op_config->read_from_ddr[0].filename, "block_op/mobilenet_bin/block%d/Block%d_Tile%d_row%d_col1_%d.bin", block_id, block_id, sub_id, row, col_1);
            op_config->read_from_ddr[0].ddr_base_address = op_config->value_at_reg_0x90;
            op_config->read_from_ddr[0].info_size = op_config->value_at_reg_0x94;
        }
    ...
    {
        sprintf(op_config->read_verify[0].filename, "block_op/mobilenet_bin/block%d/relu_outb.tile_idx_%d_%d.3.bin", block_id, row, col_1);
        sprintf(op_config->read_from_ddr[0].filename, "block_op/mobilenet_bin/block%d/Block%d_Tile%d_row%d_col1_%d.bin", block_id, block_id, sub_id, row, col_1);
        op_config->read_from_ddr[0].ddr_base_address = op_config->value_at_reg_0x90;
        op_config->read_from_ddr[0].info_size = op_config->value_at_reg_0x94;
    }
...
void set_mobilenet_parameter(op_ini_param_t *op_param)
{
    op_param->net_type = 3;
    ...
    if (op_param->v_layer == 1)
    {
        ...
        op_param->act_bank_out_seq[0] = 3; //{3, 3, 3, 3},
        op_param->act_bank_out_seq[1] = 3; //{3, 3, 3, 3},
        op_param->act_bank_out_seq[2] = 3; //{3, 3, 3, 3},
        op_param->act_bank_out_seq[3] = 3; //{3, 3, 3, 3},
    ...
    for (int col_1 = 0; col_1 < op_param->col / op_param->blockstride; col_1++)
    {
        for (int row = 0; row < op_param->row; row++)
        {
            for (int col_2 = 0; col_2 < op_param->blockstride; col_2++)
            {
                ...
                set_mobilenet_parameter(op_param);
                ...
                // {
                set_mobilenet_verify_parameter(&op_config, test_block, op_param->sub_id, row, col_1, col_2);
                // }
